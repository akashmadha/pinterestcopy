...
function ImgContainer() {
  ...
  // Preload images
  useEffect(() => {
    slides.flatMap(s => s.images).forEach(src => {
      const img = new Image();
      img.src = src;
    });
  }, []);
   ...
   useEffect(() => {
    slide.flatMap(s => s.images).forEach(src => {
      const img = new Image();
      img.src = src;
    });
  }, []);

  // Rotate slides with animation
  useEffect(() => {
    ...
    <main className="main-wrap">
      <div className="hero-container">
        ...
        <div className={`hero-subtitle ${fade ? 'fade-out' : 'fade-in'}`}>
          <h2 key={current} className="subtitle-text">{slides[current].text}</h2>
        </div>
          ...
          <div className={`images-group fade ${fade ? 'fade-out' : 'fade-in'}`}>
            {slides[current].images.map((src, j) => (
              <div className="image-bucket" key={j}>
                <img src={src || PLACEHOLDER} alt={`slide ${current} - ${j}`} draggable="false" />
              </div>
            ))}
          </div>
        </div>
